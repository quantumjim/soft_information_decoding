...
from ..Hardware.subsampling import generate_subsets_with_center, get_subsample_layout, get_cols_to_keep
from ..Stim_circuits.circuits import RepetitionCodeStimCircuit
from ..Probabilities.GaussianConvertor import gaussianIQConvertor, GMMIQConvertor
from ..IQ_data.simulator import RepCodeIQSimulator
from ..Plotting.IQ_plotter import plot_IQ_data_with_countMat
...
def decoderGMM(provider, DEVICE, LOGICAL, XBASIS, ROUNDS, file_name):
    md = load_metadata(DEVICE, LOGICAL, XBASIS, ROUNDS)
    jobs_by_calib_date = organize_jobs_by_calibration_date(md)
    ...
    for calib_date, job_ids in jobs_by_calib_date.items():
        ...
        all_memories_PS, _, msmt_error_dict = load_calibration_memory(provider, tobecalib_backend=DEVICE, 
        ...
        (countMat_PS, pSoft_PS, estim0matrix, estim1matrix, estim2matrix), gmm_dict = GMMIQConvertor(big_memory, all_memories_PS, inverted_q_map, plot=True)
        # pSoft, ratio, countMat, pSoft_PS, ratio_PS, countMat_PS = get_pSoft_and_countMat(big_memory, kde_dict, kde_dict_PS, inverted_q_map, threshold, plot = True)
        # pSoft_PS, ratio_PS, countMat_PS = get_pSoft_and_countMat(big_memory, kde_dict, kde_dict_PS, inverted_q_map, threshold, plot = True)
        # pSoft_PS, ratio_PS, countMat_PS = get_pSoft_and_countMat(big_memory, kde_dict, kde_dict_PS, inverted_q_map, threshold, plot = False)
        # pSoft_mean = np.mean(pSoft)
        ...
        # Process the pSoft_PS and countMat_PS
        mask = np.where(countMat_PS == 2)
        ratio_PS = np.sum(countMat_PS[mask]) / (countMat_PS.shape[0]*countMat_PS.shape[1])
        pSoft_PS[mask] = 0.5-1e-8 # set to ambiguous
        countMat_PS[mask] = 1 # set to one 
