...
def rx_draw(graph):
    ...
    for i, edge in enumerate(graph.edge_list()):
        ...
        weight = edge_data.get('weight', 1)  # Default weight to 1 if not available
    ...
    edge_labels = {(src, tgt): f"w:{data['weight']}, {data['qubits']}" for (src, tgt, data) in G_nx.edges(data=True)}
...
def rx_draw_2D(graph):
    ...
    for node in graph.node_indexes():
        ...
        node_info_dict[node] = {'is_boundary': is_boundary, 'time': time, 'index': index}
    ...
    for i, edge in enumerate(graph.edge_list()):
        ...
        ax.plot([src_time, tgt_time], [src_index, tgt_index], color=edge_color, linestyle='--')
    ...
    ax.legend(handles=[plt.Line2D([0], [0], marker='o', color='w', label='Boundary Qubits', markersize=10, markerfacecolor='red'),
                      plt.Line2D([0], [0], marker='o', color='w', label='Check Nodes', markersize=10, markerfacecolor='blue'),
                      plt.Line2D([0], [0], color='g', linestyle='--', label='Time edges'),
                      plt.Line2D([0], [0], color='grey', linestyle='--', label='Mixed edges'),
